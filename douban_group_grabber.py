...
for  url_index in xrange(url_index_start,6520,20):
	...
	try:
		...
		print page_url
		msg=urllib2.build_opener(urllib2.ProxyHandler(proxy_dict[random.randint(0,len(proxy_dict))]),proxy_auth_handler).open(page_url).read()
		...
		for group_id_index in xrange(group.index(group_id_start),len(group)):
			...
			while is_next_page:
				...
				group_view=urllib2.build_opener(urllib2.ProxyHandler(proxy_dict[random.randint(0,len(proxy_dict))]),proxy_auth_handler).open(group_url).read()
				...
				for topic_id_index in xrange(0,len(topic)):
					...
					while is_next_comment:
						...
						topic_view=urllib2.build_opener(urllib2.ProxyHandler(proxy_dict[random.randint(0,len(proxy_dict))]),proxy_auth_handler).open(topic_url).read()
